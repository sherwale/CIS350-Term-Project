...
public class connectGrid {
   ...
   //JButton btn = new JButton("Reset");
    ...
    public static class createGrid extends JPanel implements MouseListener {
       int currentTurn = 1;
       int rows = 6;
       int columns = 7;
       int startingX = 0;
      ...
      // JButton btn = new JButton("Reset");
       ...
       public void paintComponent(Graphics connectGraphic){
        ...
        public int checkVertical(){
           int check = 0;
           int check2 = 0;
           //Goes through the grid and checks if there is a winner vertically.
           for(int i = 0; i<cGrid.length -1; ++i){
               check = 0;
               check2 = 0;
               for(int j = 0; j<cGrid[0].length -1; ++j){
                   //if the color is red 4 in a row vertically, then red won.
                   if(cGrid[j][i].equals(new Color(255,0,0))){
                    ...
                    if(check == 4){
                       ...
                       //if the color is yellow 4 in a row vertically, then yellow won.
                       check = 0;
                       check2++;
                       if(check2 == 4){
                           return 1;
